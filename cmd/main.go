...
import (
	...
	"github.com/k8s-operators-devops/app-maintenance-operator/internal/controller"
...
func init() {
	...
	utilruntime.Must(k8smaintenancev1alpha1.AddToScheme(scheme))
...
func main() {
	...
	if len(watchNamespaces) > 0 {
		...
		setupLog.Info("Restricting manager cache to configured namespaces", "namespaces", strings.Join(watchNamespaces, ","))
	} else {
	...
	if err := (&controller.MaintenanceReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "maintenance")
		os.Exit(1)
	}
